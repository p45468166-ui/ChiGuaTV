...
function authenticate(req, res, next) {
    // 对于登录注册页面和静态资源，不需要认证
    const url = req.url;
    if (url === '/login.html' || url === '/register.html' || url.startsWith('/api/login') || url.startsWith('/api/register') || url.startsWith('/css/') || url.startsWith('/js/') || url.startsWith('/images/')) {
        return next();
    }
    ...
    // 简单的token验证（实际生产环境应该使用更安全的方式）
    if (token) {
        return next(); // 这里只是简单验证，实际应该验证token的有效性
    }
