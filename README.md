...
吃瓜TV是一个基于Node.js的视频聚合平台，集成了30+个影视资源站点的API，提供智能搜索、资源聚合、实时测速等功能，让你轻松构建个人影视库。
...
#### 3. 配置 TMDb API Key
编辑 `public/index.html` 文件，替换为你自己的 TMDb API Key：
```javascript
...
4. 创建一个新的 API 密钥（选择 "Developer" 类型）
5. 复制生成的 API 密钥
...
2. 创建一个新的 Workers 服务
3. 更新 `wrangler.toml` 文件，填写你的 Workers 服务名称：
   ```toml
...
1. **本地测试**：运行 `npm run dev` 启动本地开发服务器，访问 `http://localhost:8787` 进行测试
2. **在线测试**：部署后访问你的 Workers 服务 URL，如 `https://your-worker-name.workers.dev`
3. **调试工具**：使用 Cloudflare Workers 控制台的 "Logs" 功能查看运行日志
...
- **API 请求失败**：检查网络连接和 TMDb API Key 是否有效
- **播放失败**：检查资源站点是否可用，尝试切换其他资源源
- **登录失败**：检查用户名和密码是否正确，确认密码长度不少于6个字符
- **部署失败**：检查 `wrangler.toml` 配置是否正确，确保 Cloudflare CLI 已登录

#### 10. 高级配置

- **自定义站点**：编辑 `worker.js` 中的 `DEFAULT_SITES` 数组，添加或修改资源站点
- **修改管理员密码**：编辑 `worker.js` 中的管理员登录 API 部分
- **添加更多功能**：根据开发计划，逐步实现更多功能

### 访问应用
- 前端页面: https://your-worker-name.workers.dev
- 后台管理: https://your-worker-name.workers.dev/admin.html
- 管理员密码: `admin`
